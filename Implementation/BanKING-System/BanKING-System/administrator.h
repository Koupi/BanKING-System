...
const char* CHECK_CLIENT_REQUEST = "SELECT * FROM BANK_CLIENTS WHERE passport_number = @pasport_number;";
...
const char* UPDATE_REMOVE_OVERDRAFT_REQUEST	= "UPDATE BANK_ACCOUNTS SET overdraft_id = NULL WHERE account_number = @account_number;";
...
const char* CHANGE_ACCOUNT_TYPE_REQUEST = "UPDATE BANK_ACCOUNTS SET type_id = (SELECT id FROM ACCOUNT_TYPE WHERE `type` = @account_type), \
